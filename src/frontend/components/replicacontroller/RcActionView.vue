  ...
  return [
    {
      ...
      action: async () => {
        const resource = await K8sService.getResource({
          resType: ResType.ReplicationController,
          ns: props.rc.metadata.namespace,
          name: props.rc.metadata.name,
        })
        DrawerHelper
          .instance
          .drawer(drawer)
          .showWider(props.rc.metadata.name, MonacoView, { item: resource })
      },
    },
    {
      label: 'Delete',
