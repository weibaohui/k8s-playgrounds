...
import EChartView from '@frontend/components/common/EChartView.vue'
...
lesMiserables.nodes.forEach((node: any) => {
  node.label = {
...
const option = {
  ...
  legend: [
    {
      data: lesMiserables.categories.map((a: { name: string }) => {
        return a.name
      }),
    },
  ],
  ...
  series: [
    {
      ...
      data: lesMiserables.nodes,
      links: lesMiserables.links,
      categories: lesMiserables.categories,
      roam: true,
      label: {
        position: 'right',
        formatter: '{b}',
...
<template>
  <EChartView :option="option" width="100%" height="100%" />
</template>
