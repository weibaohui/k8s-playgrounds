...
import type { V1Role } from '@backend/k8s/model/V1Role'
...
const itemList = ref<V1Role[]>()
const searchText = ref('')
const workloadListViewRef = ref<InstanceType<typeof WorkloadListView>>()

function createColumns(): DataTableColumns<V1Role> {
  return [
    {
      type: 'selection',
    },
          ...
          NButton,
          {
            ...
            onClick: async () => {
              DrawerHelper
                .instance
                .drawer(drawer)
                .show(`${ResType.Role}:${row.metadata.name}`, RoleView, { role: row })
            },
          },
...
async function getItemList() {
  itemList.value = await K8sService.playService.roleControllerListByNs({ ns: selectedNs.value })
...
async function onRemoveBtnClicked(keys: string[]) {
  DialogHelper.instance.dialog(dialog).confirm('删除', async () => {
    await K8sService.playService.roleControllerDelete({ requestBody: keys })
  })
}
