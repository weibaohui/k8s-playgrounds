...
function getOptions(): ActionMenuOption[] {
  return [
    ...
    },
    {
      ...
      icon: Trash,
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.hpa.metadata.namespace}/${props.hpa.metadata.name}`, async () => {
          await K8sService.playService.horizontalPodAutoscalerControllerDelete({ requestBody: [`${props.hpa.metadata.namespace}/${props.hpa.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
