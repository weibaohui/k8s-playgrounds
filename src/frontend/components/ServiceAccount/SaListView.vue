...
import type { V1ServiceAccount } from '@backend/k8s/model/V1ServiceAccount'
import { ResType } from '@backend/k8s/watch/watch.model'
import { TimerUtils } from '@backend/utils/TimerUtils'
import SaActionView from '@frontend/components/ServiceAccount/SaActionView.vue'
...
const itemList = ref<V1ServiceAccount[]>()
const searchText = ref('')
const workloadListViewRef = ref<InstanceType<typeof WorkloadListView>>()

function createColumns(): DataTableColumns<V1ServiceAccount> {
  return [
    {
      type: 'selection',
    },
          ...
          NButton,
          {
            ...
            onClick: async () => {
              DrawerHelper
                .instance
                .drawer(drawer)
                .show(`${ResType.ServiceAccount}:${row.metadata.name}`, SaView, { sa: row })
            },
          },
...
async function getItemList() {
  itemList.value = await K8sService.playService.serviceAccountControllerListByNs({ ns: selectedNs.value })
...
async function onRemoveBtnClicked(keys: string[]) {
  DialogHelper.instance.dialog(dialog).confirm('删除', async () => {
    await K8sService.playService.serviceAccountControllerDelete({ requestBody: keys })
  })
}
