  ...
  return [
    {
      ...
      action: async () => {
        await DrawerHelper.instance.drawer(drawer).showResourceEditor({
          ...
          ns: props.sa.metadata.namespace,
          name: props.sa.metadata.name,
        })
      },
    },
    {
      ...
      icon: Trash,
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.sa.metadata.namespace}/${props.sa.metadata.name}`, async () => {
          await K8sService.playService.serviceAccountControllerDelete({ requestBody: [`${props.sa.metadata.namespace}/${props.sa.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
