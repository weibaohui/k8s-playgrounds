  ...
  return [
    {
      ...
      action: async () => {
        const resource = await K8sService.getResource({
          resType: ResType.Job,
          ns: props.job.metadata.namespace,
          name: props.job.metadata.name,
        })
        DrawerHelper
          .instance
          .drawer(drawer)
          .showWider(props.job.metadata.name, MonacoView, { item: resource })
      },
    },
    {
      label: 'Delete',
      ...
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.job.metadata.namespace}/${props.job.metadata.name}`, async () => {
          await K8sService.playService.jobControllerDelete({ requestBody: [`${props.job.metadata.namespace}/${props.job.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
