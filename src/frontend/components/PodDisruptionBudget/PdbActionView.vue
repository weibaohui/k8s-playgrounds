  ...
  return [
    {
      ...
      action: async () => {
        await DrawerHelper.instance.drawer(drawer).showResourceEditor({
          resType: ResType.PodDisruptionBudget,
          ns: props.pdb.metadata.namespace,
          name: props.pdb.metadata.name,
        })
      },
    },
    {
      ...
      icon: Trash,
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.pdb.metadata.namespace}/${props.pdb.metadata.name}`, async () => {
          await K8sService.playService.podDisruptionBudgetControllerDelete({ requestBody: [`${props.pdb.metadata.namespace}/${props.pdb.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
