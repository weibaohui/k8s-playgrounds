...
const itemList = ref<V1PodDisruptionBudget[]>()
const searchText = ref('')
const workloadListViewRef = ref<InstanceType<typeof WorkloadListView>>()

function createColumns(): DataTableColumns<V1PodDisruptionBudget> {
  return [
    {
      type: 'selection',
    },
    {
      title: 'Name',
      key: 'metadata.name',
      render(row: V1PodDisruptionBudget) {
        return h(
          NButton,
          {
            text: true,
            onClick: async () => {
              DrawerHelper
                .instance
                .drawer(drawer)
                .show(`${ResType.PodDisruptionBudget}:${row.metadata.name}`, PdbView, { pdb: row })
            },
          },
...
async function getItemList() {
  itemList.value = await K8sService.playService.podDisruptionBudgetControllerListByNs({ ns: selectedNs.value })
