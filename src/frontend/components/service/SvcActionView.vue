...
function getOptions(): ActionMenuOption[] {
  return [
    ...
    },
    {
      ...
      icon: Trash,
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.svc.metadata.namespace}/${props.svc.metadata.name}`, async () => {
          await K8sService.playService.networkSvcControllerDelete({ requestBody: [`${props.svc.metadata.namespace}/${props.svc.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
