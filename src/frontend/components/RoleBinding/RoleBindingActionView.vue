  ...
  return [
    {
      ...
      action: async () => {
        await DrawerHelper.instance.drawer(drawer).showResourceEditor({
          resType: ResType.RoleBinding,
          ns: props.roleBinding.metadata.namespace,
          name: props.roleBinding.metadata.name,
        })
      },
    },
    {
      ...
      icon: Trash,
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.roleBinding.metadata.namespace}/${props.roleBinding.metadata.name}`, async () => {
          await K8sService.playService.roleBindingControllerDelete({ requestBody: [`${props.roleBinding.metadata.namespace}/${props.roleBinding.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
