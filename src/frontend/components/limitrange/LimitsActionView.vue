  ...
  return [
    {
      ...
      action: async () => {
        const resource = await K8sService.getResource({
          ...
          ns: props.limits.metadata.namespace,
          name: props.limits.metadata.name,
        })
        DrawerHelper
          .instance
          .drawer(drawer)
          .showWider(props.limits.metadata.name, MonacoView, { item: resource })
      },
    },
    {
      label: 'Delete',
      ...
      icon: Trash,
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.limits.metadata.namespace}/${props.limits.metadata.name}`, async () => {
          await K8sService.playService.limitRangeControllerDelete({ requestBody: [`${props.limits.metadata.namespace}/${props.limits.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
