    ...
    },
    {
      ...
      action: () =>
        DialogHelper.instance.dialog(dialog).confirmWithTarget('删除', `${props.ep.metadata.namespace}/${props.ep.metadata.name}`, async () => {
          await K8sService.playService.endpointSliceControllerDelete({ requestBody: [`${props.ep.metadata.namespace}/${props.ep.metadata.name}`] })
          drawer.close()
        }),
    },
  ]
